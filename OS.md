...
1. **关中断** 在进入锁测试之前关闭中断，直到完成锁测试并上锁之后才能代开中断。这样，进程在临界区执行期间，计算机系统不响应中断，从而不发生调度，也就不会发生进程或线程切换。由此保证了对锁的测试和关锁操作的连续性和完整性，有效地保证了互斥。
	...
	3. 关中断不适用与多CPU系统。
2. **利用Test-and-Set指令实现互斥** 用TS指令管理临界区时，会为每个临界资源设置一个布尔变量lock，由于变量lock代表了该资源的状态，故可把它看成一把锁。lock的初值为FALSE，表示该临界资源空闲。进程进入该临界区前，首先用TS指令测试lock，如果其值为FALSE，则表示没有进程在临界区内，可以进入，并将TRUE值赋给lock，这等效于关闭了临界资源，使任何进程都不能进入临界区，否则必须循环测试直到TS(s)为TRUE。
...
## 2.27 怎么看一个进程占用了哪些端口？
## 2.28 Linux下进程与线程的区别？
## 2.29 如果要计算0-1000000所有的质数，用单线程和多线程计算有什么区别？
## 2.30 如果一个进程有异常该如何监控?
## 2.31 死锁发生的条件是什么？如何解决死锁？
### 2.31.1 可抢占性资源与不可抢占性资源
