...
4. **独立性** 在同一进程中的不同线程之间的独立性要比不同进程之间的独立性低的多。 线程有自己的栈空间和局部变量，但**线程之间没有单独的地址空间**，一个线程死掉就等于整个进程死掉，所以多进程的程序要比多线程的程序健壮。
...
2. **管道**(PIPE)通信系统
管道是指用于连接一个读进程和一个写进程以实现它们之间通信的一个共享文件。向管道(共享文件)提供输入的发送进程(写进程)以字符流形式将大量数据送入管道；而接受管道输出的接收进程(读进程)则从管道中接收数据。管道主要包括无名管道和命名管道：无名管道可用于具有亲缘关系的父子进程间的通信，命名管道除了具有管道所具有的功能外，还允许无亲缘关系进程间的通信。
	1. 普通管道PIPE
...
#### 2.7.4.2 死锁避免
1. **安全状态和不安全状态** 
	1. 安全状态是指系统能按照某种进程推进顺序为每个进程分配其所需要的资源，直至满足每个进程对资源的最大需求，使每个进程都可以顺利地完成。
	2. 系统处于安全状态就不会进入死锁状态，系统处于不安全状态就有可能进入死锁状态。
2. 当有进程请求一个可用资源时，系统需要对该进程的请求进行计算，如果将资源分配给进程后系统仍然处于安全状态，才将该资源分配给进程。

##### 2.7.4.2.1 利用银行家算法避免死锁 
1. 银行家算法中的数据结构：(n个进程，m种资源)
	1. Available[m]，可利用资源向量。Available[j] = k表示当前时刻系统中有j资源k个。
	2. Max[n, m]，最大需求矩阵。Max[i, j] = k表示进程i需要资源j的最大数目为k个。
	3. Allocation[n, m] 分配矩阵。Allocation[i, j] = k表示当前进程i已经分得资源j的数目为k个。
	4. Need[n, m]，需求矩阵。Need[i, j] = k表示进程i还需要资源jk个才能完成任务。
	5. Need[i, j] = Max[i, j] - Allocation[i, j]
2. 银行家算法
	1. Request_i是进程i的请求向量，Request_i[j] = k表示当前时刻进程i发出对资源j的k个请求，当进程发出资源请求后，系统按照如下步骤检查：
		1. 如果Request_i[j] <= Need[i, j]，就执行步骤2，否则进程i请求的资源已经超出其所宣布的最大值，出错。
		2. 如果Request_i[j] <= Available[j]，就执行步骤3，否则表示系统当前空闲资源无法满足进程i的请求，进程i等待。
		3. 系统试探把资源按照请求分配给进程i，并修改下面数据结构中的数值：
			1. Available[j] = Avaliable[j] - Request_i[j];
			2. Allocation[i, j] = Allocation[i, j] + Request_i[j];
			3. Need[i, j] = Need[i, j] - Request_i[j];
		4. 系统执行安全性算法，检查此次资源分配后系统是否处于安全状态。如果安全，才将资源正式分配给进程i，完成此次分配；否则本次试探分配作废，恢复之前的资源分配状态，进程i等到。

3. 安全性算法
	1. Work[m]工作向量，表示系统可以提供给进程继续运行所需的各类资源数目，初始值等于Available[m]。
	2. Finish[m]，表示系统是否有足够的资源分配给进程，使之运行完成，初始值全部为false，当有足够的资源分配给进程i时，再令Finish[i] = true。
		1. 从进程集合中找到一个能满足 Finish[i]=false && Need[i, j] <= Work[j]的进程，如果找到，执行步骤2，否则执行步骤3。
		2. 当进程i获得资源后，可顺利执行，直至完成，并释放出分配给它的资源：Work[j] = Work[j] + Allocation[i, j], Finish[i]=true;执行步骤1。
		3. 如果所有进程都满足Finish[i]==true, 则表示系统处于安全状态，否则系统处于不安全状态。

#### 2.7.4.3 死锁检测
1. 资源分配图：进程节点、资源节点、资源请求边、资源分配边。
2. 死锁定理：对资源分配图的所有简化顺序都将得到相同的不可简化图，S状态资源分配图为死锁状态的充分条件时：当且仅当S状态资源分配图是不可完全简化的。

#### 2.7.4.4 死锁解除
1. 抢占资源：从一个或者多个进程中抢占足够数量的资源，分配给死锁进程，以解除死锁状态。
2. 终止或者撤销进程：终止或者撤销系统中的一个或者多个死锁进程，直至打破循环环路，使系统从死锁状态解脱出来。
...
### 3.2.2 分页存储管理的地址变换机构
1. 地址变换机构的基本任务是实现从逻辑地址到物理地址的转换，由于页内地址和块内地址是一一对应的，不需要再进行转换，因此，地址变换机构的任务实际上就是将逻辑地址中的页号转换为内存中的物理块号。
2. 地址变换任务借助于页表来完成。
3. 基本的地址变换结构
	1. 页表大多驻留在内存中，但是系统中只设置一个页表寄存器，在其中存放页表在内存中的始地址和页表的长度。平时进程未执行时，页表的始地址和页表长度存放在本进程的PCB中，当调度程序调度到某进程时，才将这两个数据装入页表寄存器中。
	2. 当进程要访问某个逻辑地址中的数据时，分页地址变换机构会自动地将有效地址分为页号和业内地址两部分，再以页号为索引去检索页表，将页表始地址与页号和页表项长度的乘积相加，变得到该页项在页表中的位置，于是可以得到该页的物理块号，再将业内地址送入物理地址寄存器是我块内地址字段中，这样就完成了从逻辑地址到物理地址的转换。
	3. CPU每存取一个数据需要两次访问内存。
4. 具有快表的地址变换结构
	1. 快表(联想寄存器/TLB)，是一种访问速度比内存快很多的高速缓冲存储器，用来存放当前访问的若干页表项，以加快地址变换的过程。
	2. 当进程要访问某个逻辑地址中的数据时，分页地址变换机构会自动地将有效地址分为页号和页内地址两部分，如果在快表中找到匹配的页号，说明要访问的页表项在快表中有副本，则直接从中取出该页对应的内存块号，再将内存块号与页内地址拼接成为物理地址，最后访问该物理地址对应的单元。
	3. 如果快表中没有找到匹配的页号，则需要访问内存中的页表，同时在找到页表项之后，应将其存入快表。
